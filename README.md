...
O objetivo principal é ilustrar de forma clara como modelos teóricos se comportam na prática, permitindo ao usuário interagir com hiperparâmetros do modelo e visualizar os resultados de treinamento e previsão.
...
3.  Treinamento e comparação de dois métodos de classificação distintos:
    * **Regressão Logística (Logistic Regression - LR)**: Um modelo linear e robusto para classificação binária.
    * **Rede Neural Densa (Deep Neural Network - DNN)**: Um modelo não-linear, mais complexo, implementado via TensorFlow/Keras.
